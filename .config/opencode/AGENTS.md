...
| 3 | `src/app.constants.ts` | `FILES_BASE_URL` hardcoded to `http://localhost:3000` | High |
| 4 | `src/domain/users/auth/jwt.strategy.ts` | `ignoreExpiration: true` — tokens never expire | High |
| 5 | `src/domain/users/services/email-verification-otp.service.ts` | OTP store in-memory — lost on restart, breaks multi-instance | High |
| 6 | `src/main.ts` | Duplicate `Vary: Origin` middleware registered twice | Low |
| 7 | `src/integrations/cost-estimation.client.ts` | `console.log("aaa", ...)` debug statement | Low |
| 8 | `src/migrations/main.ts` | Entity glob points to `restaurant` instead of `domain` | Medium |
| 9 | `POST /api/v1/notifications/test-push-all` | Fully public — sends FCM to all users | High |
| 10 | `POST /api/v1/products/:id/inventory` | Fully public — directly adjusts Zoho inventory | High |
